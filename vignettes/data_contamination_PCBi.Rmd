...
PCBi <- data.frame(PARAMETRE_LIBELLE = c("CB 28", "CB 52", "CB 101", "CB 118", "CB 138", "CB 153", "CB 180"),
                   EAC_OSPAR = c(67, 108, 121, 25, 317, 1585, 469),
                   UNITE = rep.int("ng/g lw", times = 7)
)
...
# Add environmental threshold when existing => EAC OSPAR 2023
  geom_text(
    data = res_PCBi_lw_trends,  
    aes(x = last_year+1, y = max(log(last_resultat))*1.1, label = sprintf("%.2f", log(EAC_OSPAR))),
    color = "red", size = 2.5
  ) +
  geom_hline(
    data = res_PCBi_lw_trends,
    aes(yintercept = log(EAC_OSPAR)),
...
```{r}
res_kruskal_PCBi_lw <- get_kruskal_periods_results(data_with_periode = data_PCBi_lw, 
                             chemicals = PCBi$PARAMETRE_LIBELLE)
```

```{r}
plot_PCBi_lw_periode <- data_PCBi_lw |> 
  ...
  labs(x = NULL, y = "Concentration in ng/g lw", colour = "Period") +
  theme(legend.position = "bottom") +
