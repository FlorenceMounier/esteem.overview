...
In ww
```{r}
short_last_trend_PAH_ww <- res_PAH_ww_trends |> 
  filter(PARAMETRE_LIBELLE %in% PAH$PARAMETRE_LIBELLE[1:4]) |> 
  select(ESTUARY, PARAMETRE_LIBELLE, symbol) |> 
  rename(short_last_trend = symbol) 

long_term_PAH_ww <- res_kruskal_PAH_ww |> 
  ...
  select(ESTUARY, PARAMETRE_LIBELLE, trend, status) |> 
  rename(long_term_trend = trend)

general_eval_PAH_ww <- full_join(long_term_PAH_ww, short_last_trend_PAH_ww)  |>
...
writexl::write_xlsx(x = general_eval_PAH_ww, 
                    path = "../inst/results/data_contam/PAH/general_eval_PAH_ww.xlsx")
```

In dw
```{r}
short_last_trend_PAH_dw <- res_PAH_dw_trends |> 
  select(ESTUARY, PARAMETRE_LIBELLE, symbol) |> 
  rename(short_last_trend = symbol) 

long_term_PAH_dw <- res_kruskal_PAH_dw |> 
  mutate(status = case_when(
    median_2 >= 0 & median_2 < 50 ~ "blue",
    median_2 >= 50 & median_2 < 100 ~ "green",
    median_2 >= 100 & median_2 < 150 ~ "orange",
...
general_eval_PAH_dw <- full_join(long_term_PAH_dw, short_last_trend_PAH_dw) |>
  unite(col = eval, long_term_trend, short_last_trend, status, sep = "  ") |> 
  select(ESTUARY, PARAMETRE_LIBELLE, eval) |> 
  pivot_wider(names_from = ESTUARY, values_from = eval)
  
writexl::write_xlsx(x = general_eval_PAH_dw, 
                    path = "../inst/results/data_contam/PAH/general_eval_PAH_dw.xlsx")
```
