...
# Sample surface (log m²)
data_log_sample_surface <- data_benthos_ind_species_m2 |> 
  distinct(estuary, LIEU_MNEMONIQUE, DATE, sampler_type, sampler_surface, tidal, nb_samples_lieu_mnemonique_date) |> 
  mutate(log_surface_sample = log(sampler_surface * nb_samples_lieu_mnemonique_date))

# Compute species richness per log m²
data_species_richness <- left_join(data_n_species_by_sample, data_log_sample_surface) |> 
  mutate(n_species_per_log_m2 = n_species / log_surface_sample)

# Summarise per estuary and year
data_species_richness_year <- data_species_richness |> 
  mutate(YEAR = year(DATE)) |> 
  group_by(estuary, YEAR, tidal) |> 
  summarise(mean_year_n_species_per_log_m2 = mean(n_species_per_log_m2), 
            sum_log_sample_surface = sum(log_surface_sample), .groups = "drop")
```
...
## Total surface sampled variability
  ggplot_surface_variability <- ggplot(data = data_species_richness_year) +
    aes(x = YEAR, y = sum_log_sample_surface, color = estuary,
        linetype = tidal, shape = tidal) +
    geom_point() + geom_line() +
    labs(x = NULL, y = "Total surface sampled (log m²)", 
  ...
  ggplot_surface_richness_correlation <- ggplot(data = data_species_richness_year) +
    aes(x = sum_log_sample_surface, y = mean_year_n_species_per_log_m2, 
...
Plot of species richness per log m² per estuary and year:
```{r}
ggplot_species_richness_year <- ggplot(data_species_richness_year) +
  aes(x = YEAR, y = mean_year_n_species_per_log_m2, color = estuary,
      linetype = tidal, shape = tidal) +
  geom_point() +
  geom_line() +
  labs(title = "REBENT macrobenthic fauna - Species richness per log square metre", 
       x = NULL, y = "Mean number of species per log m²",
       color = "Estuary") +
  theme_esteem()

ggplot_species_richness_year
ggsave(plot = ggplot_species_richness_year, filename = "../inst/results/data_benthos/indicators/ggplot_REBENT_species_richness_log_squarem.jpg",
       width = 15, height = 6, units = "cm")
```
