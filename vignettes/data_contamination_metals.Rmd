...
```{r}
metaux <- data.frame(
  PARAMETRE_LIBELLE = c("Mercure", "Cadmium", "Plomb", "Cuivre"),
...
res_metaux_trends_2 <- res_metaux_trends |> 
  filter(PARAMETRE_LIBELLE %in% c("Cuivre"))
...
res_kruskal_metaux <- get_kruskal_periods_results(
  data_with_periode = data_metaux_ww |> filter(PARAMETRE_LIBELLE %in% c("Cuivre")), 
  chemicals = c("Cuivre"))
```
...
```{r}
data_metaux_ww_levels <- data_metaux_ww |> 
...
data_metaux_ww_norm_relative <- res_kruskal_metaux_norm_relative |> 
  select(ESTUARY, PARAMETRE_LIBELLE, median_1, median_2)

data_metaux_general_levels <- full_join(data_metaux_ww_levels, data_metaux_ww_norm_relative)

writexl::write_xlsx(x = data_metaux_general_levels, path = "../inst/results/data_contam/metals/summarised_levels_data_metaux_ww.xlsx")
```
