...
## Time series graphs
```{r}
fun_seg_model_metals1 <- function(chemical, estuary){
  fun_seg_model(data = data_metaux1,
                folder = "metals",
                chemical = chemical, 
                estuary = estuary)
}
  
metals1_names <- df_metal1$PARAMETRE_LIBELLE_EN
estuary_names <- c("Gironde", "Loire", "Seine")
params <- expand.grid(estuary_names, metals1_names)

res_metals1_trends <- pmap_dfr(params, ~ fun_seg_model_metals1(estuary = ..1, chemical = ..2))
