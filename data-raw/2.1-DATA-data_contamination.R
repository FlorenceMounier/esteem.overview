...
# From /data-raw/1.2-RAW-data_POMET_haline_zones.R
...
mutate(RESULTAT = case_when(
  UNITE == "pg.g-1, p.h." ~ RESULTAT / 1000,
...
data_2_groups <- data_contamination_no_correction |>
...
ggplot_contamination_loire_parallel_sampling <- ggplot(data_contamination_no_correction |> filter(estuary == "Loire")) +
...
df_overlap <- data_contamination_no_correction |>
...
data_contamination_full_corr <- data_contamination_no_correction |>
  left_join(
...
df_mussel_equiv_ng_gdw <- data_contamination_no_correction |>
  left_join(
    correction_table_ng_gdw |>
      select(PARAMETRE_LIBELLE, correction_factor, correction_applied),
    by = "PARAMETRE_LIBELLE"
...
data_ROCCHMV_contam_2_groups <- left_join(data_2_groups, data_contamination_no_correction)

ggplot(data_ROCCHMV_contam_2_groups |> filter(estuary == "Loire")) +
  aes(x = year, y = RESULTAT_ng_glw, colour = group) +
  geom_point() +
  geom_line() +
  facet_wrap(vars(PARAMETRE_LIBELLE), scale = "free_y")

data_contamination_convert_factors <- data_ROCCHMV_contam_2_groups |>
...
data_contamination <- data_contamination_no_correction |>
...
# =====================================================
# 06. Campaign frequency
# =====================================================

gantt_data <- data_contamination |>
  group_by(estuary, family, PARAMETRE_LIBELLE) |>
  mutate(
