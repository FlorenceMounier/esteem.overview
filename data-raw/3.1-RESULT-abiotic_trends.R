...
# ---- Trends results ----
gam_stats_abiotic <- gam_results_abiotic$stats |>
  arrange(PARAMETRE_LIBELLE, estuary, haline_zone, season)

gam_stats_abiotic_signif <- gam_stats_abiotic |>
  filter(p_value < 0.05) |>
  arrange(PARAMETRE_LIBELLE, p_value)

gam_stats_abiotic_summary <- gam_stats_abiotic |>
  count(PARAMETRE_LIBELLE, trend_type)

# ---- Plots ----
plots_results_abiotic <- plot_trends_by_parameter(
  data = data_abiotic,
  gam_results = gam_stats_abiotic,
  response_col = "RESULTAT",
  x_col = "year_month"
)
...
# ---- Trends results ----
gam_stats_contamination_ng_gdw <- gam_results_contamination_ng_gdw$stats
gam_stats_contamination_ng_gww <- gam_results_contamination_ng_gww$stats
gam_stats_contamination_ng_glw <- gam_results_contamination_ng_glw$stats


# ---- Plots ----

plots_results_contamination_ng_gdw <- plot_trends_by_parameter(
  data = data_contamination,
  gam_results = gam_results_contamination_ng_gdw,
  response_col = "RESULTAT_ng_gdw",
  x_col = "year"
)

plots_results_contamination_ng_gww <- plot_trends_by_parameter(
  data = data_contamination,
  gam_results = gam_results_contamination_ng_gww,
  response_col = "RESULTAT_ng_gww",
  x_col = "year"
)

plots_results_contamination_ng_glw <- plot_trends_by_parameter(
  data = data_contamination,
  gam_results = gam_results_contamination_ng_glw,
  response_col = "RESULTAT_ng_glw",
  x_col = "year"
)
...
gg_mercury <- plots_results_contamination_ng_gww[["Mercury"]] +
  geom_hline(yintercept = 500)  +
  theme(axis.title.x = element_blank(),
        axis.title.y = element_blank(),
        legend.position = "none")

gg_cadmium <- plots_results_contamination_ng_gww[["Cadmium"]] +
  geom_hline(yintercept = 1000)  +
  theme(axis.title.x = element_blank(),
        axis.title.y = element_blank(),
        legend.position = "none")

gg_lead <- plots_results_contamination_ng_gww[["Lead"]] +
  geom_hline(yintercept = 1500)  +
  theme(axis.title.x = element_blank(),
        axis.title.y = element_blank(),
        legend.position = "bottom")

gg_copper <- plots_results_contamination_ng_gww[["Copper"]] +
  theme(axis.title.x = element_blank(),
        axis.title.y = element_blank(),
        legend.position = "bottom")
...
# =====================================================
# 2. Long term trends correlations
# =====================================================

# ==== Abiotic data ====

# ---- Smoothed trajectories ----
gam_predictions_abiotic <- gam_results_abiotic$predictions

# ---- Correlations between GAM trajectories ----
cor_gam_abiotic <- cor_gam_trajectories(
  gam_predictions = gam_predictions_abiotic,
  group_vars = c("estuary", "haline_zone", "season"),
  method = "spearman",
  min_n = 6
)
